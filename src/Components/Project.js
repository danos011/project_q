...
export const Project = ({fadeOut}) => {
    ...
    useEffect(() => {
        preloadImage('/image/q.png');
    }, []);

    return (
        <section id={'project'} className={"text-white p-10 pt-[90px] pr-[50px] h-[100vh] flex flex-row text-center gap-3 w-full uppercase"}>
            <img src="/image/project.png" alt="project" className={"absolute right-0 top-0 right-animation"}/>
            <div className={`flex flex-col gap-10 w-[40%] items-center ${fadeOut ? 'fade-in' : ''} `}>
                <div
                    className={"uppercase text-9xl leading-10 tracking-tighter mt-[90px] font-bigstem left-animation"}>
                    we are
                </div>
            ...
            <img src="/image/we.png" alt="we" className={"absolute left-0 left-animation"}/>
        </section>
    )
}